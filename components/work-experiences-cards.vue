		...
		<article v-for="(exp, index) in experiences" :key="`${exp.company}-${index}`"
			...
			<div class="mt-3">
				...
				<transition name="collapse">
					<div v-show="isOpen(index)" :id="panelId(index)" class="mt-3">
						<ul class="list-disc px-4 space-y-1 text-slate-700 dark:text-slate-300">
							<li v-for="(item, itemIndex) in exp.bullets" :key="itemIndex">{{ item }}</li>
						</ul>
					</div>
				</transition>
			</div>
...
<script setup lang="ts">
	...
	const isOpen = (index: number) => openSet.value.has(index)
	const toggle = (index: number) => {
		openSet.value.has(index) ? openSet.value.delete(index) : openSet.value.add(index)
		openSet.value = new Set(openSet.value)
	}
	const panelId = (index: number) => `work-details-${index}`
</script>
