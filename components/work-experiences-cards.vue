<template>
	<div class="flex flex-col gap-4 mt-4">
		<article v-for="(exp, i) in experiences" :key="`${exp.company}-${i}`"
			class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
			<!-- Header -->
			...
			<div class="mt-3">
				<button v-if="exp.bullets && exp.bullets.length" type="button" class="inline-flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-medium text-white transition
								 ...
								 dark:bg-slate-200 dark:text-slate-900 dark:hover:bg-white" :aria-expanded="isOpen(i)"
					:aria-controls="panelId(i)" @click="toggle(i)">
					<span>{{ isOpen(i) ? 'Read less' : 'Read more' }}</span>
					<svg class="h-4 w-4 transition-transform" :class="{ 'rotate-180': isOpen(i) }" viewBox="0 0 20 20"
